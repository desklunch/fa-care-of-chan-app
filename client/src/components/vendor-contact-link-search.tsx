...
interface VendorContactLinkSearchProps {
  vendorId: string;
...
export function VendorContactLinkSearch({
  vendorId,
  linkedContacts,
  onLink,
  onUnlink,
  ...
  const linkMutation = useMutation({
    mutationFn: async (contactId: string) => {
      await apiRequest("POST", `/api/vendors/${vendorId}/contacts/${contactId}`);
      return contactId;
    },
    onSuccess: (contactId) => {
      ...
      toast({
        ...
        description: "Contact has been linked to this vendor.",
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  const unlinkMutation = useMutation({
    mutationFn: async (contactId: string) => {
      await apiRequest("DELETE", `/api/vendors/${vendorId}/contacts/${contactId}`);
      return contactId;
    },
    onSuccess: (contactId) => {
      onUnlink(contactId);
          ...
          <div className="relative flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-9"
                data-testid="input-vendor-contact-link-search"
              />
            </div>
            {onClose && (
              <Button
                ...
                size="lg"
                onClick={onClose}
                data-testid="button-close-vendor-contact-search"
                  ...
                  {filteredContacts.slice(0, 10).map((contact) => (
                    <li key={contact.id}>
                      <button
                        ...
                        data-testid={`option-vendor-contact-${contact.id}`}
                      >
                        <span>{contact.firstName} {contact.lastName}</span>
                        {contact.jobTitle && (
