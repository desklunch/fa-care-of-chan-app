  ...
  const columns: KanbanColumn<DealWithRelations>[] = useMemo(() => {
    ...
    return activeStatusNames.map((status) => {
      ...
      const hasCustomColor =
        record &&
        !defaultSeedColors.has(record.colorLight?.toLowerCase() || "");
      let color: string;
      if (hasCustomColor) {
  ...
  return (
    <div className="h-full">
      <Kanban
        columns={columns}
        renderCard={(deal) => (
          <DealKanbanCard deal={deal} servicesMap={servicesMap} />
        )}
...
export default function DealReports() {
  ...
  usePageHeader({
    breadcrumbs: [{ label: "Deals", href: "/deals" }, { label: "Views" }],
  });
...
function DealReportsContent({
  ...
  const getInitialTab = (): ReportTab => {
    ...
    return isValidTab(tabParam) ? tabParam : "snapshot-14";
  };
