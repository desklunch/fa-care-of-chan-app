...
const statusColors: Record<FeatureStatus, string> = {
  proposed:
    "bg-blue-600/80 text-background dark:bg-blue-400 dark:text-background",
  under_review:
    "bg-yellow-600/80 text-background dark:bg-yellow-400 dark:text-background",
  planned:
    "bg-purple-600/80 text-background dark:bg-purple-400 dark:text-background",
  in_progress:
    "bg-orange-600/80 text-background dark:bg-orange-400 dark:text-background",
  completed:
    "bg-green-600/80 text-background dark:bg-green-400 dark:text-background",
  archived:
    "bg-gray-600/80 text-background dark:bg-gray-400 dark:text-background",
};
...
function FeatureRow({ feature }: { feature: AppFeatureWithRelations }) {
  ...
  const statusMutation = useMutation({
    mutationFn: async (newStatus: FeatureStatus) => {
      const res = await apiRequest("PATCH", `/api/features/${feature.id}`, {
        status: newStatus,
      });
      return res.json();
    },
  ...
  return (
    ...
    >
      <Link
        ...
        className="flex flex-wrap items-center gap-2 px-4 py-4 hover-elevate cursor-pointer pr-28"
      >
        ...
        >
          {feature.priority && (
            <span
              ...
              data-testid={`icon-priority-${feature.id}`}
            >
...
export default function AppFeatures() {
  ...
  const [perTabFilters, setPerTabFilters] = useState<
    Record<TabValue, Record<string, string[]>>
  >(() => ({ ...initialFilterStates }));
  ...
  const activeFilters = useMemo(() => {
    ...
    return featureFilters.filter(
      (f) => f.id !== "groupBy" && f.id !== "category",
    );
  }, [isAllTab, isArchivedTab]);
  ...
  const filteredFeatures = useMemo(() => {
    return tabFilteredFeatures
      .filter((f) => {
        if (selectedStatuses.length > 0 && !selectedStatuses.includes(f.status))
          return false;
        if (
          selectedCategories.length > 0 &&
          !selectedCategories.includes(String(f.categoryId))
        )
          return false;
        return true;
      })
  ...
  const groupedFeatures = useMemo(() => {
    ...
    const groups: Record<
      string,
      { label: string; color?: string; features: AppFeatureWithRelations[] }
    > = {};
    ...
    filteredFeatures.forEach((feature) => {
      ...
      if (groupBy === "status") {
        ...
        groupLabel =
          statusLabels[feature.status as FeatureStatus] || feature.status;
      } else {
  ...
  const emptyDescription = useMemo(() => {
    if (hasActiveFilters)
      return "Try adjusting your filters to see more features.";
    if (isArchivedTab) return "Archived features will appear here.";
    if (isCategoryTab)
      return `There are no features assigned to the ${activeTab} category.`;
  ...
  return (
    ...
    >
      <div className="overflow-hidden flex flex-col h-full p-4 pt-2  gap-4 ">
        <Tabs
          value={activeTab}
        ...
        >
          <TabsList data-testid="tabs-category">
            {CATEGORY_TABS.map((tab) => {
              const value =
                tab === "All" ? "all" : tab === "Archived" ? "archived" : tab;
              return (
                <TabsTrigger
                  key={tab}
                  value={value}
