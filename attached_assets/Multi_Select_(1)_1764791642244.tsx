...
interface MultiSelectProps {
  ...
  // Data - either flat list or categorized
  items?: Item[];
...
// Shared content component used by both Dialog and DropdownMenu
function MultiSelectContent({
  ...
  const ItemCheckbox = isDropdown ? DropdownMenuCheckboxItem : "div";
  const Separator = isDropdown ? DropdownMenuSeparator : "div";
  ...
  return (
    <>
              ...
              <div className="col-span-2 relative">
                ...
                {searchQuery && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="absolute right-2 md:right-12 top-1/2 -translate-y-1/2 h-8 md:h-6 w-8 md:w-6 p-0"
      ...
      <div className="h-px bg-black/15 mx-1 mb-1" />
              ...
              <div key={category} className="mb-6 ">
                ...
                {ids.map((id) =>
                  isDropdown ? (
                    ...
                    <div
                      key={id}
                      className="relative hover:bg-black/5 flex cursor-pointer select-none items-center rounded-sm py-3 pl-12 pr-2 text-sm"
                    ...
                    >
                      <span className="absolute border border-black/10 rounded-sm left-2 flex h-6 w-6 items-center justify-center">
      ...
      <div className="h-px bg-black/15 mx-1 mt-1" />
      ...
      <div className="p-4 md:p-2">
        <Button
          variant="ghost"
          size="sm"
          className="w-full text-sm md:text-xs h-12 md:h-8 rounded-lg md:rounded-sm [&_svg]:h-4 [&_svg]:w-4 [&_svg]:stroke-[2px]"     onClick={() => setIsOpen(false)}
...
export function MultiSelect({
  ...
  // Determine if we're using categorized or flat items
  const usesCategories = !!itemsByCategory;

  // Build flat items list if using categories
  ...
  // Compute effective defaults: use all IDs if defaultSelectAll is true, otherwise use provided defaults
  ...
  // Check if current selection differs from defaults
  ...
  // Determine button variant and className
  ...
  return (
    <div className="relative inline-flex items-center ">
      {isMobile ? (
        ...
        // Mobile: Use Dialog for fullscreen
        ...
        <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DropdownMenuContent
            align={align}
            className={"bg-background border-black/10 w-72 max-h-[600px] rounded-md flex flex-col p-0 mt-1"}
      ...
      {/* Clear button - rendered outside to avoid nested buttons */}
      {isDifferentFromDefaults && (
        <Button
          variant="ghost"
          size="sm"
          className="bg-white/20 text-background absolute right-2 h-5 w-5 p-0 rounded-sm hover:bg-primary-foreground  hover:text-primary [&_svg]:h-3 [&_svg]:w-3 [&_svg]:stroke-[3px]"
          onClick={(e) => {
            e.stopPropagation();
            reset();
          }}
