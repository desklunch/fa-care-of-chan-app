...
${JSON.stringify(googlePlaceData, null, 2)}`;
  ...
  app.get("/api/clients/:id", isAuthenticated, async (req, res) => {
    try {
      const client = await storage.getClientByIdWithRelations(req.params.id);
      if (!client) {
  ...
  app.post("/api/clients", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: client.id,
        status: "success",
        metadata: { clientName: result.data.name },
      });
  ...
  app.patch("/api/clients/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: req.params.id,
        status: "success",
        changes: getChangedFields(existingClient, result.data),
      });
  ...
  app.delete("/api/clients/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: req.params.id,
        status: "success",
        metadata: { deletedClient: existingClient.name },
      });
  ...
  // Client-Contact association routes
  ...
  app.post("/api/clients/:id/contacts", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const link = await storage.addContactToClient(req.params.id, contactId);
      ...
      await logAuditEvent(req, {
        ...
        entityId: link.id,
        status: "success",
        metadata: { clientId: req.params.id, contactId },
      });

      res.status(201).json(link);
    } catch (error) {
      console.error("Error adding contact to client:", error);
      res.status(500).json({ message: "Failed to add contact to client" });
    }
  });

  app.delete("/api/clients/:clientId/contacts/:contactId", isAuthenticated, async (req: any, res) => {
    try {
      await storage.removeContactFromClient(req.params.clientId, req.params.contactId);

      await logAuditEvent(req, {
        action: "delete",
        entityType: "client_contact",
        status: "success",
        metadata: { clientId: req.params.clientId, contactId: req.params.contactId },
      });

      res.status(204).send();
    } catch (error) {
      console.error("Error removing contact from client:", error);
      res.status(500).json({ message: "Failed to remove contact from client" });
    }
  });

  app.patch("/api/clients/:clientId/contacts/:contactId", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await storage.setClientContactActive(req.params.clientId, req.params.contactId, isActive);

      await logAuditEvent(req, {
        action: "update",
        entityType: "client_contact",
        status: "success",
        metadata: { clientId: req.params.clientId, contactId: req.params.contactId, isActive },
      });

      res.json({ success: true });
    } catch (error) {
      console.error("Error updating client contact status:", error);
      res.status(500).json({ message: "Failed to update client contact status" });
    }
  });
  ...
  app.get("/api/deals", isAuthenticated, async (req, res) => {
    try {
      const { status, clientId, ownerId } = req.query;
      
      const options: { status?: DealStatus[]; clientId?: string; ownerId?: string } = {};
      
      if (status) {
        const statusArray = Array.isArray(status) ? status : [status];
        const validStatuses = statusArray.filter(s => dealStatuses.includes(s as DealStatus));
        if (validStatuses.length > 0) {
          options.status = validStatuses as DealStatus[];
        }
      }
  ...
  app.post("/api/deals", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: deal.id,
        status: "success",
        metadata: { clientId: result.data.clientId, status: result.data.status },
      });
  ...
  app.patch("/api/deals/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: req.params.id,
        status: "success",
        changes: getChangedFields(existingDeal, result.data),
      });
  ...
  app.delete("/api/deals/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      await logAuditEvent(req, {
        ...
        entityId: req.params.id,
        status: "success",
        metadata: { deletedDealClientId: existingDeal.clientId },
      });
